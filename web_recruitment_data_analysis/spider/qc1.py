...
def get(html):
    ...
    reg = re.compile(r'<div class="el">.*?<p class="t1 ">.*?href="(.*?)" onmousedown="">.*?</p>',re.S)
    items = re.findall(reg,html)
    print items #列表list
    return items

#多页,写入文件
for j in range(134,1345):
    ...
    url0 = 'http://search.51job.com/list/000000,000000,0148%252C0107%252C0117,01,9,99,%2B,2,' + str(j) + '2.html?lang=c&stype=1&postchannel=0000&workyear=99&cotype=99&degreefrom=99&jobterm=99&companysize=99&lonlat=0%2C0&radius=-1&ord_field=0&confirmdate=9&fromType=4&dibiaoid=0&address=&line=&specialarea=00&from=&welfare='
    html = get_content(url0)  # 调用获取源码
    for i in get(html):

        with open('web-mis-url.txt','a') as f:
            f.write(i+'\n')
